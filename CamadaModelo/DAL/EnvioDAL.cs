...
namespace PIM_VIII.Model
{
    public class EnvioDAL : IConnection<PIM_VII.VO.Envio>
    {
        ...
        const string _SELECT_ALL = @"SELECT tbl_cronograma.ID_CRONOGRAMA,
                                ...
                                ORDER BY tbl_cadastro_envio.DATA_ENVIO";
        const string _INSERT_ALUNO = @"INSERT INTO tbl_cadastro_envio (ID_CRONOGRAMA, MATRICULA, DATA_ENVIO, OBSERVACAO_ALUNO)
                                VALUES(?, ?, ?, ?)";
        const string _UPDATE_ALUNO = @"UPDATE tbl_cadastro_envio SET DATA_ENVIO = ?, OBSERVACAO_ALUNO = ?
                                WHERE [Código] = ?";
        ...
        public void Atualiza(Envio envio)
        {
            try
            {
                ...
                {
                    using (OleDbCommand cmd = new OleDbCommand(_UPDATE_ALUNO, conexao))
                    {
                            ...
                            if (reader.HasRows)
                            {
                                while (reader.Read())
                                {
                                    var envio = new Envio
                                    {
                                        ...
                                        nota = reader[7] == DBNull.Value ? -1 : reader.GetFloat(7)
                                    };
                                    result.Add(envio);
                                }
                            }
        ...
        public List<Envio>GetByAluno(Aluno aluno)
        {
            try
            {
                return GetAll().Where(x => x.aluno != null && x.aluno.Matricula == aluno.Matricula).ToList();
            }
            catch (Exception ex)
            {
                throw ex;
